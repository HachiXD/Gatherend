...
export const removePendingRead = (roomId: string) =>
...
function buildChannelState(
  ...
  return { unreads: nextUnreads, lastAck: nextLastAck };
}
...
export const useUnreadStore = create<UnreadState>((set, get) => ({
  ...
  getUnreadCount: (roomId) => get().unreads[roomId] || 0,
  ...
  setUnreadCount: (roomId, count) =>
    set((state) => {
      ...
      return { unreads: { ...state.unreads, [roomId]: count } };
    }),

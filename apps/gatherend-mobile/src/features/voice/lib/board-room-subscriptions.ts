...
function getBoardRoomRefCounts(socket: Socket) {
...
export function acquireBoardRoom(socket: Socket, boardId: string) {
...
export function releaseBoardRoom(socket: Socket, boardId: string) {
...
export function rejoinBoardRooms(socket: Socket) {
  const refCounts = boardRoomRefCounts.get(socket);
  if (!refCounts) return;

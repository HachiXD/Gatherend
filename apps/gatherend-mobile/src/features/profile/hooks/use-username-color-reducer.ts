...
import type {
  ...
  UsernameColor,
  UsernameColorState,
} from "@/src/features/profile/types/username-color";

const DEFAULT_USERNAME_COLOR = "#B5B5B5";
const FALLBACK_GRADIENT_COLOR = "#FF5733";

let gradientStopCounter = 0;

function createGradientStopId(): string {
  gradientStopCounter += 1;
  return `username-gradient-stop-${gradientStopCounter}`;
}
...
export function createEditableGradientColorStop(stop: {
  color: string;
  position: number;
}): EditableGradientColorStop {
  return { color: stop.color, position: stop.position, editorId: createGradientStopId() };
}

function toEditableGradientStops(
  colors: readonly { color: string; position: number }[],
): EditableGradientColorStop[] {
...
function parseInitialColor(color: unknown): UsernameColorState {
  ...
  if (typeof color === "object" && color !== null) {
    ...
    if (c?.type === "gradient") {
      const firstColor =
        normalizeUsernameGradientStops(c.colors)[0]?.color ??
        FALLBACK_GRADIENT_COLOR;

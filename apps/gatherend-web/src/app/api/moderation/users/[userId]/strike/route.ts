...
import {
  DIRECT_STRIKE_DEFAULT_SEVERITY,
  issuePlatformStrike,
  UUID_REGEX,
} from "@/lib/platform-moderation";

const STRIKE_SEVERITIES = ["LOW", "MEDIUM", "HIGH", "CRITICAL"] as const;
...
export async function POST(
  ...
  const { reason, notes, reportId, boardId, severity } = (body ?? {}) as {
    reason?: unknown;
    notes?: unknown;
    reportId?: unknown;
    boardId?: unknown;
  ...
  if (
    boardId !== undefined &&
    boardId !== null &&
    (typeof boardId !== "string" || !UUID_REGEX.test(boardId))
  ) {
  ...
  if (
    ...
    (typeof severity !== "string" ||
      !STRIKE_SEVERITIES.includes(severity as (typeof STRIKE_SEVERITIES)[number]))
  ) {
  ...
  try {
    const [targetProfile, report] = await Promise.all([
      ...
      reportId
        ? db.report.findUnique({
            where: { id: reportId },
            select: { id: true, targetOwnerId: true, boardId: true, snapshot: true, targetType: true },
          })
    ...
    const result = await db.$transaction(async (tx) =>
      issuePlatformStrike(tx, {
        profileId: targetProfile.id,
        issuedById: admin.profile.id,
        reason: reason.trim(),
        severity:
          typeof severity === "string" ? severity : DIRECT_STRIKE_DEFAULT_SEVERITY,
        notes: notes?.trim() || null,
        reportId: report?.id ?? null,
        boardId: typeof boardId === "string" ? boardId : report?.boardId ?? null,
        originReportId: report?.id ?? null,
        snapshot: (report?.snapshot as object | null) ?? null,
        contentType: report?.targetType?.toLowerCase() ?? "manual",
      }),
    );
    ...
    console.error("[MODERATION_USER_STRIKE_CREATE]", error);
    return NextResponse.json(

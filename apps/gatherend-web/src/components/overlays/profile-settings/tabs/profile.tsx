      ...
      <div className="space-y-1.5">
        ...
        <div className="grid grid-cols-1 gap-3 md:grid-cols-2">
          <div className="space-y-0.5">
            ...
            <div className="flex overflow-hidden rounded-md border border-theme-border" role="group">
              <button
                type="button"
                disabled={isSaving}
                onClick={() =>
                ...
                className={cn(
                  "flex h-8 w-full cursor-pointer items-center justify-center px-3 text-[13px] transition disabled:cursor-not-allowed disabled:opacity-70",
              ...
              <button
                ...
                className={cn(
                  "flex h-8 w-full cursor-pointer items-center justify-center border-l border-theme-border px-3 text-[13px] transition disabled:cursor-not-allowed disabled:opacity-70",
                  !value.roundedEnabled
                    ? controlButtonActiveClass
                    : controlButtonInactiveClass,
                )}
...
export const ProfileTab = ({ user }: ProfileTabProps) => {
  ...
  const [chatBubbleStyle, setChatBubbleStyle] = useState<ChatBubbleStyle>(
    () =>
      normalizeChatBubbleStyle(extendedUser.chatBubbleStyle) ?? {
        ...EDITOR_DEFAULT_CHAT_BUBBLE_STYLE,
      },
  );
  ...
  const onSubmit = useCallback(
    async (values: FormSchema) => {
      ...
      try {
        ...
        setChatBubbleStyle(
          normalizeChatBubbleStyle(serverProfile.chatBubbleStyle) ?? {
            ...EDITOR_DEFAULT_CHAT_BUBBLE_STYLE,
          },
        );
